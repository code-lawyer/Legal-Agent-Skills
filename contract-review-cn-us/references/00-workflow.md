...
- 若合同类型未在任何 `_pack.md` 登记表命中（无对应领域卡，如股权等暂未建卡的类型）：不报错也不停，按 `_general.md` + 02 骨架做通用审查，并在审查备注注明"本类型无专项审查卡，仅通用审查，专项漏检风险需人工补足"。
...
- 通用安全线（任何交付都适用）：禁止自动改事实/金额/法律依据/核心条款，只能提建议交律师/用户定。
- 用户要 Word 红线稿才读 `08-redline.md`，否则不加载；红线场景的完整自动修正边界与七步 QA 见 08。
