...
- **未接 MCP 不静默出正式报告**：未探测到任何法律库 MCP 时，`00-routing-intake.md` 的「MCP 前置门」会**先引导接入、暂停正式产出**，而非"照常出报告只是标未验证"；唯一例外是用户被告知后仍显式要求的、通篇水印「未接法律库·仅初步参考·不可作为诉讼或交付依据」的降级草稿。硬前置逻辑见 `00-routing-intake.md` 与 `10-retrieval-core.md`，本文件只负责"怎么接"。
