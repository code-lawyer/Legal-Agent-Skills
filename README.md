...
把"资深执业律师会怎么做"沉淀成 agent 可直接执行的结构化流程，覆盖三类高频法律工作：
...
三个 skill 边界互斥、彼此路由：合同相关 → `contract-review`；法律问题论证或类案检索 → `legal-research`；按案卷/知识库填充文书模板 → `document-fill`。均**不做**合同起草、企业核验、尽调合规、续约提醒、流程图。
...
**1. 防幻觉是第一设计目标。**
法条/案例不能靠模型记忆——那是最危险的幻觉源。`contract-review`、`legal-research` 都要求每条法律命题**挂且仅挂一个来源标签**（`[MCP核验:…]` / `[模型知识-未验证]` 等），标签描述"实际来源行为"而非"自信程度"；三轴正交（验证渠道 / 权威位阶 / 处置状态），互不冒充。`document-fill` 走的是同一防幻觉哲学的另一种落地：每个填入值挂且仅挂一个状态标签（`extracted`/`inferred`/`user_confirmed`/`ambiguous`/`gap`/`pending_drafting`），且 `extracted` 值必须能在来源原文中逐字命中（Mode-2）。
