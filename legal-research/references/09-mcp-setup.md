...
本 skill 对 MCP **松耦合**：接上任何专业法律库 MCP 即走 `[MCP核验:…]` 实时核验；未接则回退模型知识（`[模型知识-未验证]`）、高风险点提示人工核实，纯分析路径**零外部依赖**、不因未接而拒绝作业。级联逻辑见 `10-retrieval-core.md`，不写死任何具体 MCP。

本文件只提供一个**推荐源的现成接入方式**，方便使用者一键接上、把结论从"模型知识"升级为"已核验"。用不用、用哪家都由使用者决定。
